...
async function main() {
  ...
  // Check if git is initialized
  if (!isGitRepository()) {
    console.log(`${colors.yellow}⚠ Not a git repository. Skipping deployment.${colors.reset}\n`);
    rl.close();
    process.exit(0);
  }
  ...
  if (status.modified > 0) console.log(`  ${colors.yellow}Modified:${colors.reset} ${status.modified} file(s)`);
  if (status.added > 0) console.log(`  ${colors.green}Added:${colors.reset} ${status.added} file(s)`);
  if (status.deleted > 0) console.log(`  ${colors.red}Deleted:${colors.reset} ${status.deleted} file(s)`);
  console.log();
